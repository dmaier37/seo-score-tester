...
export default function ScoreForm({ onSubmit, error }: Props) {
  const [form, setForm] = useState<AuditData>({ url: '', businessName: '', keyword: '', location: '', email: '' })
  const [loading, setLoading] = useState(false)

  const handle = (e: React.ChangeEvent<HTMLInputElement>) =>
    setForm(f => ({ ...f, [e.target.name]: e.target.value }))

  const focus = (e: React.FocusEvent<HTMLInputElement>) => e.target.style.borderColor = 'var(--accent)'
  const blur = (e: React.FocusEvent<HTMLInputElement>) => e.target.style.borderColor = 'var(--border)'

  const submit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.url || !form.businessName || !form.email) return
    setLoading(true)
    onSubmit(form)
  }
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '3rem 1.5rem' }}>
        <div style={{ maxWidth: '580px', width: '100%', textAlign: 'center' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', background: 'rgba(108,71,255,0.12)', border: '1px solid rgba(108,71,255,0.3)', borderRadius: '100px', padding: '0.3rem 0.9rem', marginBottom: '1.5rem' }}>
            ...
            <span style={{ fontSize: '0.75rem', color: '#a78bfa', fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase' }}>Free SEO Audit — Real Results</span>
          </div>
          ...
          <h1 className="font-display" style={{ fontSize: 'clamp(2rem, 5vw, 3.25rem)', fontWeight: 800, lineHeight: 1.1, letterSpacing: '-0.03em', marginBottom: '1rem' }}>
            Find out why your<br /><span style={{ color: 'var(--accent)' }}>competitors outrank you</span>
          </h1>

          <p style={{ color: 'var(--muted)', fontSize: '1.05rem', lineHeight: 1.6, marginBottom: '2.5rem' }}>
            We crawl your live website and run 15+ real SEO checks. Get your actual score in under 30 seconds — no signup, no credit card.
          </p>
          ...
          <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem', textAlign: 'left' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.875rem' }}>
              <div>
                ...
                <input name="url" value={form.url} onChange={handle} onFocus={focus} onBlur={blur} required placeholder="https://yoursite.com" style={inputStyle} />
              </div>
              <div>
                <label style={labelStyle}>Business Name *</label>
                <input name="businessName" value={form.businessName} onChange={handle} onFocus={focus} onBlur={blur} required placeholder="Acme Plumbing Co." style={inputStyle} />
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.875rem' }}>
              <div>
                <label style={labelStyle}>Target Keyword</label>
                <input name="keyword" value={form.keyword} onChange={handle} onFocus={focus} onBlur={blur} placeholder="e.g. plumber near me" style={inputStyle} />
              </div>
              <div>
                <label style={labelStyle}>Location</label>
                <input name="location" value={form.location} onChange={handle} onFocus={focus} onBlur={blur} placeholder="e.g. Miami, FL" style={inputStyle} />
              </div>
            </div>
            <div>
              <label style={labelStyle}>Your Email * <span style={{ color: 'var(--muted)', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>(we'll send your full report here)</span></label>
              <input name="email" type="email" value={form.email} onChange={handle} onFocus={focus} onBlur={blur} required placeholder="you@yourbusiness.com" style={inputStyle} />
            </div>

            {error && <p style={{ color: '#ff4757', fontSize: '0.875rem', textAlign: 'center' }}>{error}</p>}

            <button type="submit" disabled={loading || !form.url || !form.businessName || !form.email}
              style={{ marginTop: '0.5rem', background: loading ? 'var(--surface2)' : 'var(--accent)', color: 'white', border: 'none', borderRadius: '12px', padding: '1rem 2rem', fontSize: '1rem', fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', transition: 'all 0.2s', letterSpacing: '-0.01em', fontFamily: 'Space Grotesk, sans-serif' }}>
          ...
          <div style={{ marginTop: '2rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
            {['Real site crawl', 'Results in 30 seconds', '100% free'].map(t => (
